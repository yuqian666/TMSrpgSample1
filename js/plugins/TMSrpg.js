 ...
 * ==================================================================================
 == Terms of Use ==
 1. 本插件是对作者 tomoaky (http://hikimoki.sakura.ne.jp/)插件的深入修改版。

 2. 免费游戏(非商业)可以免费使用本插件

 2. 商业游戏要使用本插件需要购买商业版，商业版拥有更高级界面，配套技能装备插件等，价格68元。

 3. 限于本人能力和时间，不对本插件的bug承诺修改（但我很乐意修改，请发现后告知我），也不因此接受退款要求。

 4. 无论普通版和商业版都允许自行任意修改，但不允许将修改后的插件分享或者出售给其他人，如果有更新成果，欢迎与我分享。

 5. 作者邮箱1021539858#qq.com，哔哩哔哩主页 https://space.bilibili.com/22912906

 6. 关注作者网页，有购买链接以及更多自制插件 https://rpg.blue/home.php?mod=space&uid=18091
...
(function() {
  ...
  Game_Map.prototype.onSrpgTurnStart = function() {
    ...
    if (this._srpgActorTurn) {
  	...
  	//i do
  	$gameMap.actorEvents = $gameMap.srpgActorUnits(true);
...
Game_Event.prototype.list = function() {
  ...
  // 範囲内のSRPGユニットを返す
  Game_Map.prototype.srpgUnitsArea = function(area, alive,ev) {

    area = area || this._srpgArea;
    var result = [];

	
  var flag = ev.isSrpgActorUnit();



	var events = this.srpgUnits(alive);

	var events2 =  events.filter(item => {return item._srpgActorId != null;}); 
	//this.srpgActorUnits(alive);
	//this.srpgUnitsAreaActor(area, true,this);

	var events3 = events.filter(item => {return item._srpgEnemyId != null;}); 
	//this.srpgEnemyUnits(alive);
  //i do
  
  var traitors = events.filter(item => {return item.isTraitor()});


      var cc = events2.concat(traitors);  //合并数组
 
      var ee = new Set(cc); //通过set集合去重
 
      var ff = Array.from(ee);

      events2 = ff;


  //this._events3 = events3;


    var st = $gameMap._curentSkillType;


    // if(Scene_Map.prototype._srpgStatusWindow)
    // 	st = Scene_Map.prototype._srpgStatusWindow._actionObject.damage.type;//_curentSkillType;
    ...
    if(flag)	//是actor
    {
		...
		if(st==1 || st==2)	//战斗
		{
		    ...
		    d = //events3;
		    events3.filter(item => {
		    	return !item.isTraitor()  ;
		    });;
		    //d = c.filter(item => {return item._srpgActorId == null;}); 
		    //a = a.filter(item => { return !b.includes(item._eventId); });
			console.log(events3);
		}
		
		else if(st==3 || st==4)	//是回血
		{
			console.log("2");
			d  = events2;
		    //d = a.filter(item => { return !b.includes(item._eventId); });
			//console.log(a);
		}
		else
		{
    ...
    else //enemy
  	{
		if(st==1 || st==2)
		{
			console.log("4"); 
		    //var c = events3;
		    //d = a.filter(item => { return !c.includes(item._srpgActorId); });
			//d = a.filter(item => { return d.includes(item._eventId); });//
			
			// 找出中立阵营的货色
			var j = events.filter(item => { 
				return (ev && ev.event() && ev.event().meta.neutral);
				//return ( ev && (ev._srpgEnemyId == 5));
			});;


			// 过滤出enemy和非叛徒
			var j2 = events.filter(item => { 

				return ((item._srpgActorId == null) && !item.isTraitor());

			});;


			// 取上面反的集合，即actor和叛徒
			var h = events.filter(item => { return !j2.includes(item._srpgEnemyId);  });
			...
			var c = h.concat(j);  //合并数组
 
			var e = new Set(c); //通过set集合去重
 
			var f = Array.from(e);

			d = f;
				// d = f.filter(item => {
				// 	if(item._srpgEnemyId == 5)
				// 	{
				// 		if(ev) f.remove(ev);//ev._srpgEnemyId = null;
		...
		else if(st==3 || st==4)	//是回血
		{
			console.log("5");
			//d = events;
			d = events.filter(item => { 
				return  ( !item.isTraitor() && !item._srpgActorId) 
			});

			$gameMap._curentSkillType = 1;
...
console.log("☆☆☆☆☆☆☆☆☆☆");
  ...
  // 返回事件搜索范围内是否存在敌对单位, 返回是否有在事件的搜索操作距离敌方单位
  Game_Event.prototype.checkSearch = function() {
    ...
    var events = $gameMap.srpgUnitsArea(area, true,this);
  ...
  Game_Event.prototype.backToLastPosition = function() {
  	//i do
    if(this._lastPosition && this._lastPosition.x && this._lastPosition.y) 
  	{
  ...
  Game_Event.prototype.showSrpgActionAnimation = function() {
    ...
    var targets = $gameMap.srpgUnitsArea(null, true,this);
  ...
  Game_Event.prototype.executeSrpgAction = function() {
    ...
    //i do
    var checkFlag = action && action.isSkill() && action.item().id === checkSkillId;

    //i do
    var aflag =  action && action.isSkill() && action.item().id === 1;//是普通攻击
    ...
    if(flag  && !this._moved )
    {
        if(aflag && !this._srpgArea)
        {  
            console.log('-------------------------------------------------------');
               //i do 原地打怪
            $gameMap.showWaitingArea(this);
        }
        else
        {
            console.log('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++');
    ...
    //i do ★ 
    var targets = $gameTemp.yellowTiles;//$gameMap.srpgUnitsArea(null, true,this);//$gameMap._events3;
  ...
  //i do创建按键
  Window_SrpgStatus.prototype.createButtons = function() {
      ...
      button.setHotFrame(0, 0, buttonWidth, buttonHeight);
      ...
      button.setClickHandler(this.onButtonDown2.bind(this));

  };

  Window_SrpgStatus.prototype.onButtonDown2 = function() {
      var event  = this.user();
      // 切换角色
      // 静态变量c计数
      Window_SrpgStatus.prototype.onButtonDown2.c = Window_SrpgStatus.prototype.onButtonDown2.c || event._srpgActorId;
      Window_SrpgStatus.prototype.onButtonDown2.c++;

      var num = $gameMap.actorEvents.length;

      if($gameMap.actorEvents.length>0)
      {

        var nextEvent = $gameMap.actorEvents[(Window_SrpgStatus.prototype.onButtonDown2.c)%num];

        if(nextEvent)
        {
            SceneManager._scene._srpgCommandWindow.deactivate();
            SceneManager._scene._srpgCommandWindow.close();
            SceneManager._scene._srpgStatusWindow.setSrpgUnit(nextEvent);
            $gameTemp.setSrpgNextUnitId(nextEvent.eventId());
            $gamePlayer.setSrpgCameraEvent(nextEvent);      // カメラ移動

            SceneManager._scene.openSrpgCommand(nextEvent,true);
        }
      }


      // i do 非常关键
         //SceneManager._scene.areaSelecterDeactivate(true);  
        // SceneManager._scene._srpgCommandWindow.refresh();
  ...
  Window_SrpgStatus.prototype.refresh = function() {
    ...
    //i do
    var faceWidth = Window_Base._faceWidth;
    var x = this.textPadding() + faceWidth;
  ...
  Window_SrpgStatus.prototype.makeDamageText = function(targetBattler, action) {
    ...
    if (cnt > 0) result += ' / 反击(' + Math.floor(cnt * 100) + '%)';
...
//
  ...
  Window_SrpgRegionEffect.prototype.initialize = function(wy) {
    ...
    Window_Base.prototype.initialize.call(this, wx, 0, ww, wh);
    this._stateId = -1;
    this.update();
  };
  ...
  Window_SrpgRegionEffect.prototype.refresh = function() {
    ...
    this.contents.clear();
    this.drawTextEx('\\C[12]地形\\C[0]', 0, 0)
    this.drawText(state ? state.name : '无', 44, 0, this.contents.Width - 48);
  };
	...
	// };
	//标准字体大小
	Window_SrpgRegionEffect.prototype.standardFontSize = function() {
	    return 16;
	};
  ...
  Scene_Map.prototype.createSrpgCommandWindow = function() {
    ...
    // i do
    var ww = Graphics.boxWidth -this._srpgCommandWindow.width;
    var wh = (Graphics.boxHeight- this._helpWindow.height - this._srpgStatusWindow.height-this._srpgCommandWindow.height)/2;
    srpgCommandX = ww;
    srpgCommandY = wh;
  ...
  Scene_Map.prototype.createSrpgSkillWindow = function() {
    ...
    this._srpgSkillWindow = new Window_SrpgSkillList(wx, wy, ww, wh);
  ...
  Scene_Map.prototype.updateSrpgActorTurn = function() {
    ...
    console.log('ssxx'+this._srpgTurnState);
  ...
  Scene_Map.prototype.updateSrpgInput = function() {
    if (TouchInput.isRepeated()){
      ...
      if (event && TouchInput.isTriggered()) {
        this._srpgTurnState = 0;
        this.openSrpgCommand(event, true);  // 打开单位命令
  ...
  // i do
  Scene_Map.prototype.openSrpgCommand2 = function(event, showArea) {
    // //
    // this._srpgStatusWindow.setSrpgUnit(event);
    //  if (event) {
    //    this._srpgCommandWindow.setSrpgUnit(event);
  ...
  Scene_Map.prototype.srpgCommandMoveCancel = function() {
    var event = this._srpgStatusWindow.user();
    //i do
    if(event && event.isSrpgActorUnit())
    {
  ...
  Scene_Map.prototype.srpgCommandWaiting = function() {
    ...
    event._direction = 2;
  	...
  	$gameMap.hideSrpgArea(); 

  	event.endSrpgAction();   // 行動を終了する
  	event._moved = true;
  	event._acted = true;


		// if($gameMap.actorEvents.length==1)
	// {
	// 	// i do 非常关键
		
	// 	battler.onTurnEnd();
	// 	this._srpgCommandWindow.activate();
	// 	event.endSrpgMove();
	// 	event.endSrpgAction();
	// this.openSrpgCommand(event,false);
	// }


  //自动切换恢复这里	
	
	//console.log($gameMap.actorEvents.length);

	// if($gameMap.actorEvents.length>0)
	// {
	// 	//先把当前的删除
	// 	$gameMap.actorEvents.remove(event);

	// 	var nextEvent = $gameMap.actorEvents[0];//$gameMap.event(2);

	// 	if(nextEvent)
	// 	{
	// 		this._srpgCommandWindow.deactivate();
	// 	    this._srpgCommandWindow.close();
	// 	    this._srpgStatusWindow.setSrpgUnit(nextEvent);
	//     	$gameTemp.setSrpgNextUnitId(nextEvent.eventId());
	//     	$gamePlayer.setSrpgCameraEvent(nextEvent);      // カメラ移動

	//     	this.openSrpgCommand(nextEvent,true);
	// 	}
	// 	//再把当前的删除
	// 	//$gameMap.actorEvents.remove(nextEvent);
	// 	//this.openSrpgCommand(nextEvent,true);
	// }
	// if($gameMap.actorEvents.length==0)
	// {
	...
	// event._acted = true;
	// 				//this._srpgCommandWindow.deactivate();
	// 	    this._srpgCommandWindow.refresh();
	//     //this.openSrpgCommand(nextEvent,false);
  ...
  Scene_Map.prototype.srpgCommandActionYes = function() {
    var event = this._srpgStatusWindow.user();
    // console.log($gamePlayer._x+','+event._x);
    // if(($gamePlayer._x != event._x) && ($gamePlayer._y != event._y))
    // {
    // 	alert('sb');
    // }
    ...
    //i do
  	//如果用完技能后还能移动,就跳回event，并且打开命令窗口
  	if(this._srpgCommandWindow)
  	{
  		var canMove = this._srpgCommandWindow._srpgUnit.canSrpgMove();
  	  var canAct  = this._srpgCommandWindow._srpgUnit.canSrpgAct();
	    //
	  	if(canMove)
	  	{
			//alert(canMove);//!canAct &&
  ...
  Scene_Map.prototype.srpgCommandActionNo = function() {
    ...
    event.setDirection(2);//this._lastUnitDirection);    // 向きを元に戻すevent.setDirection(this._lastUnitDirection);    // 向きを元に戻す
    $gamePlayer.setSrpgCameraEvent(event);          // カメラ移動
    this._srpgCommandWindow.close();
    this._srpgStatusWindow.setSrpgUnit(event);
  };
  ...
  Scene_Map.prototype.okSrpgSkill = function() {
    ...
    this.setAreaHandler(this.okAreaAction, this.cancelAreaAction);
    $gameMap.showRangeArea(event.x, event.y, skill);
    $gameMap._curentSkillType = skill.damage.type;
...
// i do
  ...
  Scene_Map.prototype.okAreaAction = function(x, y) {
    ...
    for (var i = targets.length - 1; i >= 0; i--) {
      if(targets[i].isSrpgEnemyUnit() && !targets[i].isTraitor())
      {
        c++;
      }
    }
    ...
    if(targets.isEmpty() ) //范围内能被打的一个都没有
    {
      //i do
      event._acted = false;
      this.openSrpgHelp('', 2);
      this._srpgSkillWindow.activeOpen();
      //播放失败音
      SoundManager.playBuzzer();              // 決定効果音を鳴らす
      //鼠标回player
      $gamePlayer.setSrpgCameraEvent(event);

      this.areaSelecterDeactivate(true);
    }
    else
    {
      //alert(x+','+y);//这里的xy是目标的坐标
      //得到技能类型
      //alert( $gameMap._curentSkillType);
  ...
  Scene_Map.prototype.setSrpgAction = function(event, skill, x, y) {
    ...
    var arrs = $gameMap._srpgArea;

    //i do 这是选择的范围
    console.log($gameMap._srpgArea);

    //这是测算出来的event
    var events = $gameMap.sortEventsDistance($gameMap.srpgUnitsArea(null, true,event), x, y);
    ...
    // });
    console.log(events);//这里的events就是你选择的黄色释放点造成的点们
    
    $gameTemp.yellowTiles = events;
  ...
  Scene_Map.prototype.updateSrpgEnemyTurn = function() {
    ...
    //i do
	if(event){
			//$gameMap._curentSkillType = $dataSkills[event.srpgBattler().attackSkillId()].damage.type;
			//console.log("★★★★★★★★★★★★");
			if(this._srpgStatusWindow._actionObject)
			$gameMap._curentSkillType = this._srpgStatusWindow._actionObject.damage.type;
    	;};
    switch (this._srpgTurnState) {
    case 1:   // 移動前行動（準備）
