...
/*:
 * @plugindesc タイルトナー 1.1.2
 * マップのタイルを染色します。
 * @author サンシロ https://twitter.com/rev2nym
 * @version 1.1.2 2016/12/28 消去されるべきスプライトが残る不具合を修正。
...
Sanshiro.TileToner.version = '1.1.2';
...
Sprite_TileTone.prototype.createBitmap = function() {
    var bitmap = ImageManager.cache.getItem(this.imageCacheKey());
    if (!bitmap) {
        bitmap = new Bitmap(this.toneWidth(), this.toneHeight());
        bitmap.fillAll(this.baseColor());
        ImageManager.cache.setItem(this.imageCacheKey(), bitmap);
    }
    return bitmap;
};
...
Sprite_LowerTileTone.prototype.setPosition = function(x, y, z) {
    this.x = x + (this._baseSprite.width * this.anchor.x);
    this.y = y + (this._baseSprite.height * this.anchor.y);
    this.z = (z === undefined ? this.z : z);
};
